...
# Constants for the environment configuration
WINDOW_SIZE = (1024, 512)  # Dimensions of the game window (width, height)
JUMP_DURATION = 12  # Duration of the jump (in frames)
JUMP_VEL = 100  # Jump velocity (used for calculating jump trajectory)
OBSTACLE_MIN_CNT = 400  # Minimum obstacle spawn count for new obstacle to appear
MAX_SPEED = 100  # Maximum speed the dinosaur can run at
MAX_CACTUS_SPAWN_PROB = 0.7  # Maximum probability for spawning a cactus
BASE_CACTUS_SPAWN_PROB = 0.3  # Base probability for spawning a cactus
BIRD_SPAWN_PROB = 0.3  # Probability for spawning a bird
RENDER_FPS = 15  # Frames per second for rendering
COLLISION_THRESHOLD = 20  # Collision threshold (for accurate collision detection)
DIFFICULTY_INCREASE_FREQ = 20  # Frequency of increasing game difficulty

# Enum for different actions the dinosaur can take
class Action(int, enum.Enum):
    STAND = 0
    JUMP = 1
    DUCK = 2

# Enum for different states of the dinosaur
...
class GameMode(str, enum.Enum):
    NORMAL = "normal"
    TRAIN = "train"  # In 'train' mode, the agent receives rewards for passing obstacles, not for survival.

# Enum for different render modes (used for visualization)
class RenderMode(str, enum.Enum):
    HUMAN = "human"
    RGB = "rgb_array" # This mode returns RGB frames for training purposes


# Class to handle the game's assets (images)
class Assets:
    def __init__(self):
        # Load track image
        self.track = pygame.image.load(os.path.join("assets", "Track.png"))

        # Load dinosaur images (running, ducking, jumping)
        self.dino_runs = [
        ...
        # Load cactus images (both large and small)
        self.cactuses = [
...
# Abstract base class for in-game objects
class EnvObject(ABC):
    rect: pygame.Rect # The rectangular area of the object (used for collisions)

    def __init__(self, assets: Assets, *args, **kwargs):
        pass  # Initialize object properties

    def step(self, *args, **kwargs):
        pass  # Update the object's state (e.g., position)

    def render(self, canvas: pygame.Surface, *args, **kwargs):
        pass  # Draw the object on the screen

# Class for obstacles (inherits from EnvObject)
class Obstacle(EnvObject, ABC):
    needs_collision_check = True # Flag to indicate if the obstacle needs collision checking
...
# Track class to manage the running track background
...
class Env(gym.Env):
    ...
    def __init__(
        ...
        train_frame_limit=1000,  # the upper limit for number of frames during the train mode
    ) -> None:
...
class Wrapper(gym.Wrapper):
    ...
    def _transform(self, obs: np.ndarray) -> np.ndarray:
        ...
        return np.array(Image.fromarray(obs).convert("L").resize(self.image_size), dtype=np.float32)
